...
/***/ function(module, exports, __webpack_require__) {
	...
	var onSignUpClick = function onSignUpClick(event) {
	  $('#signUpModal').show();
...
/***/ function(module, exports, __webpack_require__) {
	...
	var setResultMessage = function setResultMessage(msg) {
	  document.getElementById('GameResultModalMessage').innerText = msg;
	};

	var setWinStatX = function setWinStatX(msg) {
	  document.getElementById('game-stats-X').innerText = msg;
	};

	var setWinStatO = function setWinStatO(msg) {
	...
	var restStats = function restStats() {
	  document.turn = 'X';
	  document.winner = null;
	  xWinCount = 0;
	  oWinCount = 0;
	...
	var initalizeGameboard = function initalizeGameboard() {
	  clearboard();
	  document.winner = null;
	  setMessage('Player "' + document.turn + '" - It\'s your turn to start.');
	};
	...
	var nextMove = function nextMove(square) {
	  if (document.winner != null) {
	    ...
	    var updateValue = document.turn;
	    var squareId = square.id;
	    var indexValue = squareId.replace('s', '');
	    ...
	    api.updateGame(data).then(ui.updateGameSuccess).then(ui.updateGameFailu);
	...
	var checkForWinner = function checkForWinner(move) {
	  var result = false;
	...
	var checkGameOver = function checkGameOver() {
	  ...
	  if (document.getElementById('s0').innerText !== '' && document.getElementById('s1').innerText !== '' && document.getElementById('s2').innerText !== '' && document.getElementById('s3').innerText !== '' && document.getElementById('s4').innerText !== '' && document.getElementById('s5').innerText !== '' && document.getElementById('s6').innerText !== '' && document.getElementById('s7').innerText !== '' && document.getElementById('s8').innerText !== '') {
	    result = true;
	  }
	  return result;
	};

	var winnerFound = function winnerFound() {
	  $('#gameboard').show();
	  ...
	  // send update request to api for game
	  api.updateGame(data);
	  api.index().then(ui.getStatsSuccess).catch(ui.getStatsSuccess);
	};

	var printWinStats = function printWinStats() {
	...
	var tiedGame = function tiedGame() {
	  gameOver = 'gameOver';
	  ...
	  api.index().then(ui.getStatsSuccess).catch(ui.getStatsFailure);
	};
	...
	var checkRow = function checkRow(a, b, c, move) {
	  var result = false;
	...
	var getBox = function getBox(number) {
	  return document.getElementById('s' + number).innerText;
	};

	var clearBox = function clearBox(number) {
	  document.getElementById('s' + number).innerText = '';
	};

	var returnToLogIn = function returnToLogIn() {
	  $('#signUpModal').hide();
	  $('sign-in').trigger('reset');
	  $('#signInModal').show();
	};
	...
	module.exports = {
	  addHandlers: addHandlers,
	  initalizeGameboard: initalizeGameboard,
	  OnNewGame: OnNewGame,
	  newGame: newGame,
	  restStats: restStats
	};
